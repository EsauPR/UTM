...
##########################################################
##  Retropropagación                                    ##
##  Actualizar la capa de entrada  multiples pesos      ##
...
def propagation( p, n, q, W_input, W_hiddens, W_output, X ) :
	global Errors
	global ents
...
def backpropagation( basePath, p, n, q, T, miu ) :
	...
	W_input = myLib.randomMatrix( p, 784 )
...
def test( filename, p, n, q ) :
	W_input = myLib.loadMatrix("Capas/CapaEntrada")
...
def main() :
	...
	if( sys.argv[1] == "-training" ) :
		W_input, W_hiddens, W_output = backpropagation( sys.argv[2], int( params[0] ), params[1], int( params[2] ), int( params[3] ), params[4] )

		# Guardando Parámetros de las neuronas de cada capa
		myLib.cleanSavesDirectory()
		myLib.saveMatrix("Capas/CapaEntrada", W_input)
